<template>
  ...
  >
    <template v-for="article in articles" :key="article.id">
      <RouterLink :to="`/article/preview/${article.id}`" target="_blank" rel="noopener noreferrer">
        <div :class="['article', `article--${defaultLayout}`]">
          <!-- 缩略图 -->
          <div
            :class="['article__thumbnail', { 'article__thumbnail--vertical': 'vertical' === defaultLayout }]"
            :style="{ height: defaultHeight }"
          >
            <!-- 图片 -->
            <img :src="article.cover" object-cover rd-4 />
            <!-- 颜色混合层 -->
            <div style="background: var(--main-gradient)" rd-4 op-50 mix-blend-screen z-1 />
            <!-- 遮盖层 -->
            <div
              :style="{
          ...
          <div flex="~ col grow-1" mx-6>
            ...
            >
              <h1
                :style="{
                  ...
                  fontSize: 'horizontal' === defaultLayout ? '1.625rem' : '2.5rem',
                }"
              >
                {{ article.title }}
              </h1>
            ...
            <div style="color: var(--text-dim)" flex="~ items-center" m="t-auto b-4">
...
import { type Article } from '@/views/website/blog/article/article';
...
const defaultHeight = computed(() => props.height ?? ('horizontal' === defaultLayout.value ? '15rem' : '30rem'));
</script>
...
@media (max-width: 992px) {
  .article-container--horizontal {
    grid-template-columns: repeat(calc(var(--article-column) - 1), minmax(0, 1fr));
  }
}
